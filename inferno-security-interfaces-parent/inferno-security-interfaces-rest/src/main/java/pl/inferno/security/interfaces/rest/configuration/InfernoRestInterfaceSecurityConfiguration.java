...
public class InfernoRestInterfaceSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	http.requestCache().and()
		...
		// .requestCache()
		// .and()
		.csrf().disable().exceptionHandling().and().anonymous().and().servletApi().and().headers()// .defaultsDisabled()
		// .xssProtection().disable()
		.cacheControl().and().addHeaderWriter(new CacheControlHeadersWriter()).and()
		// .addHeaderWriter(new
		// StaticHeadersWriter(HeaderWriterFilter.ALREADY_FILTERED_SUFFIX, ""))
		...
		.permitAll().antMatchers(HttpMethod.GET, "/api/encrypt**").permitAll()
		...
		// upon authentication
		.addFilterBefore(new InfernoLoginFilter("/api/login", tokenAuthenticationService, userDetailsService,
			authenticationManager(), passwordEncoder()), UsernamePasswordAuthenticationFilter.class)

		// custom Token based authentication based on the header previously given to the
		// client
		.addFilterBefore(new InfernoAuthenticationFilter(tokenAuthenticationService),
