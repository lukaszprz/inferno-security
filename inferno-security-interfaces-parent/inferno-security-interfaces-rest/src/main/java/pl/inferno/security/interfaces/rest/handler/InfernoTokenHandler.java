 ...
 * File: InfernoTokenHandler.java
 * Package: pl.inferno.security.interfaces.rest.handler
 * Location:
 *
 * 28 lis 2017 18:26:16 by Łukasz Przesmycki (lukasz.przesmycki@gmail.com)
 * &copy;2017
 */
package pl.inferno.security.interfaces.rest.handler;
...
/**
 * Class InfernoTokenHandler
 *
 * @author Łukasz Przesmycki (lukasz.przesmycki@gmail.com)
 *
 */
public class InfernoTokenHandler {
	...
	public InfernoTokenHandler(byte[] secretKey) {
		try {
		...
		if ((parts.length == 2) && (parts[0].length() > 0) && (parts[1].length() > 0)) {
			try {
				...
				if (validHash) {
					...
					if (new Date().getTime() < user.getExpires().getTime()) {
						return user;
					}
				}
	...
	public static void main(String[] args) {
		...
		InfernoTokenHandler tokenHandler = new InfernoTokenHandler(secret);
